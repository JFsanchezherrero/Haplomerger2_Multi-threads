...
## bug fixed
## 1djust the phase for gene prediction
## 3. to skip outputing the excised tandem sequences; output the excised sequences to STDERR
...
print STDERR "\n######## A NEW RUN ( $run_id ) of tandem removal ##########\n";
	...
	while($is_the_end==0){
		...
		if(! exists($sc{$name})){
			...
			for(my $i=$sc{$name}->[0];$i<=$sc{$name}->[1];$i++){
				...
				if($tandems[$i]->[2]<$para{'--minLen'} and $tandems[$i]->[4]<$para{'--minLen'}){ # XHM change "or" to "and", require both smaller than minLen to trigger
					$tandems[$i]->[10]=0;
				#}elsif($tandems[$i]->[6]+$tandems[$i]->[7]>$tandems[$i]->[8]+$tandems[$i]->[9]){ #old, delete the effectively shorter one
		...
		for(my $i=0;$i<scalar(@new_seq);$i++){
			if($i==0){
			  ...
			  # bug fixing 3: output the excised sequences to STDERR
   			print STDERR "\n>excised_RUNID".$run_id."_".$name."_".$i."\n";
			  $len=length($new_seq[$i]);
			  while($len>0){ print STDERR substr($new_seq[$i],0,100,''),"\n"; $len-=100;}
			  print STDERR "\n"; 
			}
		}
	}
