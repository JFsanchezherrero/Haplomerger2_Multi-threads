...
## Version 2.60
## No longer output the optiNewScaffolds_altN.fa.gz because of not much use.

## Version 2.50
## Former file name is HM_haploMerger.pl; the new codes are marked with "XHM_".
## Now able to output the optiNewScaffolds.fa.gz, optiNewScaffolds_alt.fa.gz and optiNewScaffolds_altN.fa.gz
## Current recommendation is to switch on --selectLongHaplotype but not --gapFilling.
## 
...
## bug
## 	In some cases, the output scaffold sequences may have leading or trailing Ns. 
...
if (@ARGV < 1 || $Arg_list =~ m/--help|-\?/) { print <<'USAGES' and exit; }
  ...
  NOTE: this script output the major and the alternative scaffold set.
        The portions failed to incorporated into the major set are
        put back to the alternative set with linking of single allele or N.
        ::: *_alt.fa.gz (single allele); *_altN.fa.gz (fill with N)
...
of the assembly. The script can also output unpaired sequences.
  ...
  There can be several important output files:
  1) optiNewScaffolds.fa.gz:
     A gzipped fasta file containing the reference haplome assembly that
     1) selects one representative from two haplotypes, 
     2) extends scaffold length by connecting old scaffolds from different
     haplotypes based on overlapping alignments,
     3) avoids switchs of different haplotypes inside a scaffold.
  2) optiNewScaffolds_alt.fa.gz:
  	 A gzipped fasta file contains the alternative haplome assembly.
  	 Those allelic sequences not entered the optiNewScaffolds.fa.gz have been
  	 placed in this assembly.
  	 For the loci with only one allele avialable, the reference alleles are used.
  	 Therefore, this haplome assembly share some allele sequences with
  	 the reference assembly.
  3) optiNewScaffolds_altN.fa.gz (obsolete):
  	 A gzipped fasta file contains the alternative haplome assembly.
  	 Those allelic sequences not entered the optiNewScaffolds.fa.gz have been
  	 placed in this assembly.
  	 For the loci with only one allele avialable, they are masked with Ns.
  	 Therefore, this haplome assembly contains only the allele sequences
  	 not incorporated in the reference assembly.  	    
  4) unpaired.fa.gz, contains raw unpaired sequences.
  
  5) assembly_hapA.fa.gz. original haplotype A assembly (i.e., the target assembly).
  6) assembly_hapB.fa.gz. original haplotype B assembly (i.e., the query  assembly).
      
  7) hm.unpaired_updated, describe the unpaired sequences.
  8) hm.connections, describe the connections used for optiNewScaffolds.fa.gz.
  
  There will one more output file when invocate --haploMergingMode=updated:
  9) hm.new_scaffolds_updated 
...
sub output_optimal_assembly(){
	...
	#### output primary optimal scaffold
	...
	#### output alternative scaffold for XHM_ ##################
	open(my $optFH,"|gzip -c >$out_dir/optiNewScaffolds_alt.fa.gz") or die "Can't create pipe |gzip -c >$out_dir/optiNewScaffolds_alt.fa.gz !\n";	
	for(my $i=0;$i<scalar(@opt_nsc);$i++){
		my $temp='';
		for(my $j=$opt_nsc[$i]->[0];$j<=$opt_nsc[$i]->[1];$j++){
			#$portions[$j]->[22]=$i;
			if($portions[$j]->[8]!=0 and $portions[$j]->[14]!=0){
			  $temp .= $portions[$j]->[23] == 1 ? $portions[$j]->[21] : $portions[$j]->[20];
			}else{
			  $temp .= $portions[$j]->[23] == 1 ? $portions[$j]->[20] : $portions[$j]->[21];			
			}
		}
	...
	#### output alternativeN scaffold for XHM_ ##################
	#open(my $optFH,"|gzip -c >$out_dir/optiNewScaffolds_altN.fa.gz") or die "Can't create pipe |gzip -c >$out_dir/optiNewScaffolds_altN.fa.gz !\n";	
	#for(my $i=0;$i<scalar(@opt_nsc);$i++){
	#	my $temp='';
	#	for(my $j=$opt_nsc[$i]->[0];$j<=$opt_nsc[$i]->[1];$j++){
	#		#$portions[$j]->[22]=$i;
	...
	#		}else{
	#		  if($portions[$j]->[23] == 1){
	#		    $temp.= 'N' x length($portions[$j]->[20]);
	#		  }else{
	#		    $temp.= 'N' x length($portions[$j]->[21]);
	#		  }			
	#		}
	#	}
	#	$zero_fill='0' x (7-length($i));
