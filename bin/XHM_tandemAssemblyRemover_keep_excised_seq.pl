...
## bug fixed
## 1. 部分alignment的长度比例完全不符合正方形, 处理方法是改进minInterval的判断(and to or)
## 和增加minXversusY的参数, 控制长宽比>3/4,即75%, 这样可能去掉10%的点
## 2. adjust the phase for gene prediction
## 3. not to skip outputing the excised tandem sequences, but name the original seq as "_0".
## 目的是保留所有序列, 包括切下来达到, 同时保证下一轮切割不会发生同名错误
## 现在就是: scaffoldXXX_0_1 这样的结构

## 20120916
## remove the limit, now can set $para{'--minLen'} >=500
## fix the crash when there are no tandem errors.
...
It requires chain file named self.rbest.chain.gz!
  ...
  --minXversusY=<75-100> - x to y ratio                                   
...
#### set run id # bug fixing 3: output the excised sequences to STDERR
my $run_id=int(rand(10000)); 
...
#### parse the arguments
my %para = ("--filter" => 2500, "--minLen" => 10000, "--maxInterval" => 50, "--runChainNet"=>0, #interval 25%
						"--minCoverage"=>66, "--Force" => 0, "--Delete" => 0, "--verbose" => 0, "--noRerun" => 0, '--minXversusY' => 75  );
...
#$para{'--minLen'} =3000 if $para{'--minLen'}<3000; # new XHM modified, allow 3000 #20120916
...
sub find_tandems(){
	...
	if($para{'--noRerun'}==0){
		...
		unlink glob "$out_dir/selfOnly.nets/*.*";
		rmdir "$out_dir/selfOnly.nets";
		if(! -f "$out_dir/all.rbest.chain.gz"){ die "$out_dir/all.rbest.chain.gz is not found!\n"; }
			...
			for(my $k=$i+1;$k<=$sc{$s}->[1];$k++){
				...
				if( $ct5>$nq5-$nql*0.1 and $ct5<$nq5+$nql*0.1 and $cq5>$nt5-$ntl*0.1 and $cq5<$nt5+$ntl*0.1
						...
						for($yy=$k;$yy<=$sc{$s}->[1];$yy++){
							if( $blocks[$xx]->[4]>$blocks[$yy]->[8]-($blocks[$yy]->[8]-$blocks[$yy]->[7])*0.1 and $blocks[$xx]->[4]<$blocks[$yy]->[8]+($blocks[$yy]->[8]-$blocks[$yy]->[7])*0.1
									...
									if($i!=$xx){
										...
										$found_flag=0 if $tcov/($blocks[$xx]->[4]-$blocks[$i]->[3])<$para{'--minCoverage'} or $qcov/($blocks[$xx]->[8]-$blocks[$i]->[7])<$para{'--minCoverage'}; #XHM set to or
										...
										$found_flag=0 if $tcov/($blocks[$yy]->[4]-$blocks[$k]->[3])<$para{'--minCoverage'} or $qcov/($blocks[$yy]->[8]-$blocks[$k]->[7])<$para{'--minCoverage'};	#XHM set to or								
									}
					...
					if($found_flag>0){
						...
						if( $nt5-$ct3<$ctl*$para{'--maxInterval'} and $nt5-$ct3<$cql*$para{'--maxInterval'}  #XHM intervals 25%
								and $nt5-$ct3<$ntl*$para{'--maxInterval'} and $nt5-$ct3<$nql*$para{'--maxInterval'} #XHM interval 25%
								and (($ctl>=$ntl and $ntl/$ctl>=$para{'--minXversusY'}/100) or ($ntl>=$ctl and $ctl/$ntl>=$para{'--minXversusY'}/100)) #XHM x-y ratio
								){
	...
	#### adjust phase 
	%sc=();
	if(scalar(@tandems)>0){ #only when there are tandems existed
		@tandems = sort { $a->[0] cmp $b->[0] or $a->[1] <=> $b->[1] } @tandems;
		$temp=$tandems[0]->[0];
		$sc{$temp}->[0]=0;
		for(my $i=0;$i<scalar(@tandems);$i++){
			$temp1=$tandems[$i]->[2]-int($tandems[$i]->[2]/3)*3;
			#adjusting phase# $tandems[$i]->[2] = $temp1==1 ? $tandems[$i]->[2]-1 : $temp1==2 ? $tandems[$i]->[2]-2 : $tandems[$i]->[2];
			$temp1=$tandems[$i]->[4]-int($tandems[$i]->[4]/3)*3;
			#adjusting phase# $tandems[$i]->[4] = $temp1==1 ? $tandems[$i]->[4]-1 : $temp1==2 ? $tandems[$i]->[4]-2 : $tandems[$i]->[4];	
...
sub remove_tandems(){
	...
	if($para{'--runChainNet'}==0){
		...
		open(my $tandemFH,"<$out_dir/hm.tandem_assemblies") 
			or die "Can not read $out_dir/hm.tandem_assemblies,\n to recreate this file please run the script with option --runChainNet=1 !\n";
		while(<$tandemFH>){
			next if m/^#|^\s/;
			chomp;
			push @tandems,[ split /\t/ ];
	...
	open(my $outFH,"| gzip -c >$Species[0]_rt.fa.gz") or die "Can not create: | gzip -c >$out_dir/$Species[0]_rt.fa.gz !\n";
	...
	while($is_the_end==0){
		...
		if(! exists($sc{$name})){
			...
			for(my $i=$sc{$name}->[0];$i<=$sc{$name}->[1];$i++){
				...
				if($tandems[$i]->[2]<$para{'--minLen'} and $tandems[$i]->[4]<$para{'--minLen'}){ # XHM 由or 改为 and, 要求两者都小于minLen才触发
					$tandems[$i]->[10]=0;
				#}elsif($tandems[$i]->[6]+$tandems[$i]->[7]>$tandems[$i]->[8]+$tandems[$i]->[9]){ #old, 被XHM所改写为谁的有效长度短就删除
				}elsif($tandems[$i]->[2]-$tandems[$i]->[6]-$tandems[$i]->[7] >= $tandems[$i]->[4]-$tandems[$i]->[8]-$tandems[$i]->[9]){
					$tandems[$i]->[10]=3;
				}else{
					$tandems[$i]->[10]=1;
				} 
		...
		for(my $i=0;$i<scalar(@new_seq);$i++){
			if($i==0){
				...
				#next; ## to skip outputing the excised tandem sequences
				print $outFH ">$name"."_$i"."_tag".$run_id."\n";
			}
