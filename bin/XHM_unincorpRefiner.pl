...
if (@ARGV < 1 || $Arg_list =~ m/--help|-\?/) { print <<'USAGES' and exit; }
	...
	!!!!!!This script is to refine the degenerate sequence dataset!!!!!!
  ...
  --minLenOutput=<XX>, the minimum length for output, default=300   #only for XHM               
...
my %para = ("--maskFilter" => 85,  "--redundantFilter" => 85,	"--Force" => 0, "--Delete" => 0, "--runLastzChainNet" => 0,
						"--threads"=>1, "--identity"=>50, "--minLenOutput"=>500  );
...
if($para{"--runLastzChainNet"}>0){
	...
	$cmd.= " 1>_un_initiation.log 2>>_un_initiation.log";
	system($cmd);
	print "Finish ",$cmd,"\n";
	...
	$cmd.= " --Force --Delete 1>_un_all_lastz.log 2>>_un_all_lastz.log";
	system($cmd);
	print "Finish ",$cmd,"\n"; 
	...
	$cmd.= " --Force --Delete 1>_un_axtChainRecipBestNet.log 2>>_un_axtChainRecipBestNet.log";
	system($cmd);
	print "Finish ",$cmd,"\n";	
...
for(my $i=0;$i<scalar(@unincorps);$i++){
	...
	if($unincorps[$i]->[4]<$para{'--minLenOutput'}){ $unincorps[$i]->[10]=0; next; }
	if($unincorps[$i]->[4]-$unincorps[$i]->[6]-$unincorps[$i]->[7] <$para{'--minLenOutput'}){ $unincorps[$i]->[10]=0; next; }
	if($unincorps[$i]->[4]-$unincorps[$i]->[6]-$unincorps[$i]->[8] <$para{'--minLenOutput'}){ $unincorps[$i]->[10]=0; next; }	
...
print "# and only sequences with length >= $para{'--minLenOutput'} bp were output. \n\n";
...
if($para{"--Delete"}>0){
	print "Delete optiNewScaffolds.seq, unicorporated.seq, unicorporated.optiNewScaffolds.result.\n";
	system("rm -f -r unpaired.seq optiNewScaffolds.seq unicorporated.optiNewScaffolds.result");
}
